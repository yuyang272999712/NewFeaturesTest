...
/**
 * Created by yuyang on 2017/3/23.
 */

public class ClockViewMiui extends View {
    /* 亮色，用于分针、秒针、渐变终止色 */
    private int mLightColor = Color.parseColor("#ffffff");
    /* 暗色，圆弧、刻度线、时针、渐变起始色 */
    private int mDarkColor = Color.parseColor("#80ffffff");
    /* 背景色 */
    private int mBackgroundColor = Color.parseColor("#237EAD");
    /* 小时文本字体大小 */
    private float mTextSize = DensityUtils.dp2px(getContext(), 14);
    /* 时钟半径，不包括padding值 */
    ...
    public ClockViewMiui(Context context) {
        this(context, null);
    }

    public ClockViewMiui(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ClockViewMiui(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ClockViewMiui, defStyleAttr, 0);
        mBackgroundColor = a.getColor(R.styleable.ClockViewMiui_backgroundColor, mBackgroundColor);
        mLightColor = a.getColor(R.styleable.ClockViewMiui_lightColor, mLightColor);
        mDarkColor = a.getColor(R.styleable.ClockViewMiui_darkColor, mDarkColor);
        a.recycle();
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        mCanvas = canvas;
        setCameraRotate();//设置3D效果
        getTimeDegree();//获取当前时间对应的表针偏移量
        drawTimeText();//画最外圈的小时
        drawScaleLine();//画秒刻度
        drawSecondHand();//秒针
        drawHourHand();//时针
        drawMinuteHand();//分针
        invalidate();//毫秒级的更新，来实现秒针的运动
    }
    ...
    private void drawMinuteHand() {
        mCanvas.save();
        mCanvas.translate(mCanvasTranslateX * 2f, mCanvasTranslateY * 2f);//分针在上，偏移的大一点
    ...
    private void drawHourHand() {
        mCanvas.save();
        mCanvas.translate(mCanvasTranslateX * 1.2f, mCanvasTranslateY * 1.2f);//时针在下，偏移的小一点
    ...
    private void drawSecondHand() {
        ...
        mSecondHandPath.reset();//秒针三角形的路径
        float offset = mPaddingTop + mTextRect.height() / 2;
    ...
    private void drawScaleLine() {
        ...
        mScaleArcPaint.setShader(mSweepGradient);//将着色器赋值给paint
    ...
    private void setCameraRotate() {
        ...
        mCanvas.concat(mCameraMatrix);//matrix与canvas相关联(忘了的话可以参考SimpleUseActivity.java)
    }
