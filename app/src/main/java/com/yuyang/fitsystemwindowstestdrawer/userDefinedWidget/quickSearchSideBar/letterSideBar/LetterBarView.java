 ...
 * 
 * 可用于需要按照字母排序的应用中.
 * 若排序的 item 首字为中文时，需要将每个 item 的首个汉字转化为拼音然后进行排序.
 * 将汉字转换为拼音可用 pinyin4j.jar 来转换
 *
 * ZHU yuyang 当点击导航条的字母时，会触发监听事件,若要监听，则可以使用 {@link #setOnLetterSelectListener(OnLetterSelectListener)} 来设置
 *
